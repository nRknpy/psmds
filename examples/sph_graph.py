    ...
    if n_components == 3:
        if t == 'sphere':
            ...
            if isinstance(fig, plt.Figure):
                ...
                fig.write_html(img_path / 'sph_graph_S^2.html')
        else:
            fig = scatter_euclid3d(Y, backend='plotly')
            if isinstance(fig, plt.Figure):
                fig.savefig(img_path / 'sph_graph_R^3.png')
            else:
                fig.write_html(img_path / 'sph_graph_R^3.html')
    else:
        if t == 'sphere':
            ...
            if isinstance(fig, plt.Figure):
                ...
                fig.write_html(img_path / 'sph_graph_S^1.html')
        else:
            fig = scatter_euclid2d(Y, backend='plotly')
            if isinstance(fig, plt.Figure):
                fig.savefig(img_path / 'sph_graph_R^2.png')
            else:
                fig.write_html(img_path / 'sph_graph_R^2.html')
